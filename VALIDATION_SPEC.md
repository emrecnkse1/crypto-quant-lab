...
### 17.2 CPCV — Fold Model (§17.2.1–17.2.10, §28.O — 17/17), Train-Seçimli Path Getirileri (§17.2.11–17.2.19, §28.P — 18/18) Betimsel Path Dağılımı Özeti (§17.2.20–17.2.24, §28.Q — 10/10) ve Girdi Uygunluk Denetimli Offline CPCV Çalışması + Paylaşılan Pencere Purge'ü (§17.2.26–17.2.33, §28.R — 12/12) LOCKED VE IMPLEMENTED + TESTED; Gözlem Düzeyi (t1) Purging AÇIK (veri gereksinimi §17.2.32)
...
**17.2.19 Açık kalanlar:** label/outcome-horizon purging; path Sharpe dağılımının özet istatistikleri/yorumu (ör. dağılım momentleri, DSR ile ilişki) ayrı kontrat; FAZ6C'nin diğer maddeleri (§23). [2026-09-25 notu: deterministik betimsel özet §17.2.20–17.2.24'te eklendi; yorum/eşik yine YOK.]

**17.2.20 Path Dağılımı Özeti — Kapsam (LOCKED; offline araştırma teşhisi)**

```
summarize_cpcv_paths(result: CpcvResult) -> CpcvPathSummary (cpcv.py, additive;
  compute_cpcv_paths DEĞİŞMEDİ). Yalnız betimsel: path sayısı, path Sharpe'ları,
  sıralama, min, max, ortalama, medyan, örneklem (n-1) standart sapması ve
  bağımlılık/sınır bayrakları. Eşik, p-değeri, güven aralığı, bağımsız örnek
  varsayımı, DSR/PBO birleştirmesi, geçti/kaldı YOK. Sonuç, sabit
  DIAGNOSTIC_SCOPE metnini taşır: offline araştırma teşhisi; path'ler aynı
  gözlemleri yeniden kullanır (bağımsız örnek DEĞİL); purge yalnız pencere
  düzeyi embargo, label/outcome-horizon purging YOK; tam CPCV doğrulaması ve
  canlı strateji onayı DEĞİL.
```

**17.2.21 Alanlar ve Tanımlar (LOCKED)**

```
diagnostic_scope, path_count (P), observations_per_path (T),
path_sharpe_ratios (path_index sırası; CpcvPathResult.sharpe_ratio aynen),
sorted_path_indices (Sharpe artan; eşit değerler path sırasını korur — stable),
minimum/maximum_sharpe_ratio, mean_sharpe_ratio (Σ/P), median_sharpe_ratio
(P tek: orta; P çift: iki ortadakinin ortalaması), sample_stdev_sharpe_ratio
(Σ(x-ort)²/(P-1) karekökü; Stage-2 §15.16 ile aynı n-1 kuralı; eşit
Sharpe'larda tam 0 — tanımlıdır), tie_averaged_path_count (en az bir eşitlik
ortalamalı grubu olan path sayısı), rows_identical_on_all_paths (her path'te
aynı getiriye sahip satır sayısı — path'ler arası bağımlılığın görünür ölçüsü),
paths_share_observations = True (her path her satırı kapsar),
label_horizon_purging_applied = False (model başka değeri REDDEDER).
Ortalama/medyan/stdev sabit 28 basamaklı PBO/Stage-2 context'inde; ambient
context etkisiz.
```

**17.2.22 Geçersiz / Tanımsız (LOCKED, exact mesajlar)**

```
1 result tipi         TypeError "result must be a CpcvResult, got <tip>"
2 P < 2               ValueError "a path distribution needs at least 2 paths, got P
                      (test_group_count = 1 is plain K-fold with a single path)"
                      — dağılım tanımsızdır; None/0 döndürülmez (Stage-2 ilkesi)
3 path sırası         ValueError "paths must be ordered by path_index 0..P-1"
4 path uzunluğu       ValueError "path p has n returns, expected T (one per matrix row)"
Path Sharpe'ları CpcvPathResult tarafından zaten sonlu doğrulanır; sıfır
stdev'li path compute_cpcv_paths'te reddedilir (§17.2.14 adım 10).
```

**17.2.23 Kanıt:** §28.Q.

**17.2.24 Kanıtlamadıkları:** path Sharpe'larının bağımsızlığı, istatistiksel anlamlılık, aşırı uyumun yokluğu, kârlılık; CPCV'nin, FAZ6C'nin veya Faz 6'nın tamamlanması.

**17.2.25 Label/Outcome-Horizon Purging — Ufuk Bilgisi Kaynak İncelemesi ve Sonraki Dilim Önerisi (2026-09-25; KOD YOK)**

```
Kaynak                              Taşıdığı ufuk                  CPCV'ye ulaşıyor mu
BacktestPolicy / PositionTarget     yok (salt mum-öneki)            —
BacktestResult                      yalnız fill_count/trade_count   HAYIR: fill zamanları yok;
                                    + equity eğrisi                 işlem tutma süresi geri
                                                                    kazanılamaz
ContextAwareWindow.context_start    GERİYE DOĞRU özellik ufku       HAYIR: WindowResult yalnız
  (rolling.py, çağıran beyanı)      (lookback = evaluation.start    değerlendirme penceresini
                                    - context_start), gerçek        tutar; TrialGroup/Matrix'te
                                                                    yok
TrialReturnMatrix window_indices    her rolling penceresi ayrı,     EVET: satırların hangi
  + observation_times               düz başlayan bağımsız backtest  backtest penceresinden
                                    (pozisyon pencereler arası      geldiği ve pencere sınırları
                                    taşınmaz)                       kayıtlı
Tek-bar getiri r_t, sahiplik        getirinin kendi aralığı         EVET: (t - tf, t] her zaman
  start < t <= end                  (t - timeframe, t]              tek grubun içinde
```

Bulgular: (1) Getirinin kendi aralığı grup sınırını hiç aşmaz. (2) İleriye dönük gerçek "sonuç ufku" pozisyonun tutulduğu süredir; bir fold grubu sınırı bir rolling backtest penceresinin İÇİNE düşerse, bir gruptaki karar sonraki grubun getirisini üretebilir (pozisyon taşınır) — bu klasik label örtüşmesidir ve süresi kayıtlı değildir. Sınır pencere sınırıyla çakışırsa her backtest düz başladığı için bu kanal kapalıdır. (3) Geriye dönük kanal lookback'tir; gerçek değeri ContextAwareWindow'da vardır ama CPCV girdisine taşınmaz.

Önerilen sonraki dilim (varsayımsal ufuk ÜRETMEZ, çalışan sözleşmeleri değiştirmez):

```
a) Pencere-hizalı grup denetimi: compute_cpcv_paths'e additive, saf bir kontrol —
   her fold grubu sınırının TrialReturnMatrix window_indices blok sınırıyla
   çakıştığını doğrula; bir sınır bir backtest penceresinin içine düşerse
   (pozisyon taşınabilir, ufuk bilinmez) belirli hata veya açık bayrak
   (karar: varsayılan davranış ret mi bayrak mı — öneri: ret, eski davranış
   isteğe bağlı parametreyle).
b) Lookback-embargo denetimi: çağıranın rolling'de kullandığı
   ContextAwareWindow'ları açık girdi olarak alan saf yardımcı
   (ör. required_context_embargo(windows) = max(evaluation.start -
   context_start)); fold_model.embargo bundan küçükse belirli hata. Ufuk
   çağıranın gerçekten kullandığı pencerelerden gelir; uydurulmaz. WindowResult/
   Trial sözleşmeleri DEĞİŞMEZ.
Bu iki denetimden sonra pencere düzeyinde purge "label örtüşmesi olamayacak
şekilde hizalı" hâle gelir; gözlem düzeyi t1 tabanlı klasik purging yine
(fill zamanı kaydı olmadan) uygulanamaz.
```

[2026-09-25 notu: öneri (a) ve (b) §17.2.26–17.2.33'te uygulandı; gözlem düzeyi purging için veri gereksinimi §17.2.32.]

**17.2.26 Offline CPCV Çalışması — Uçtan Uca Akış (LOCKED VE IMPLEMENTED + TESTED; §28.R)**

```
Modül: src/crypto_quant_lab/validation/cpcv_study.py (YENİ)
run_cpcv_study(group: TrialGroup, *, test_group_count, embargo=timedelta(0),
    lookback_windows: tuple[ContextAwareWindow, ...] | None = None,
    fold_groups: tuple[TemporalWindow, ...] | None = None,
    risk_free_per_period=Decimal(0), purge_shared_backtest_windows=False) -> CpcvStudy
  TrialGroup -> build_trial_return_matrix -> fold grupları (varsayılan:
  window_aligned_fold_groups(grubun kendi değerlendirme pencereleri)) ->
  build_combinatorial_fold_model -> assess_cpcv_inputs -> compute_cpcv_paths ->
  summarize_cpcv_paths (en az 2 path varsa). Değerlendirme pencereleri
  TrialGroup'un kendisinden okunur (Bölüm 20.8 aşama 13 tüm denemelerde aynı
  pencereleri zorunlu kılar).
CpcvStudy(scope, assessment, matrix, fold_model, result | None, summary | None)
window_aligned_fold_groups(windows): pencere başına bir grup [w_i.start,
  w_{i+1}.start), son grup [start, end); pencereler arası boşluk önceki gruba
  düşer ve satır taşımaz; kronolojik/örtüşmesiz ve >= 2 pencere zorunlu.
```

**17.2.27 Girdi Uygunluk Denetimi (LOCKED)**

```
assess_cpcv_inputs(matrix, fold_model, *, evaluation_windows,
    lookback_windows=None, purge_shared_backtest_windows=False) -> CpcvInputAssessment
Her kontrol: passed | failed | not_evaluated + neden (CpcvInputCheck).
runnable = hiçbiri failed değil; fully_evaluated = hepsi passed.
1 provenance.evaluation_windows  matris pencere blokları == değerlendirme
                                 pencereleri, satır satır (start < t <= end)
2 rows.fold_groups               her satır tam bir fold grubunda; her grup satırlı
3 outcome_horizon.backtest_windows  İLERİ yön (bkz. 17.2.28)
4 lookback.context               GERİ yön (bkz. 17.2.29)
Bir kontrol failed olursa bağımlı sonrakiler not_evaluated (neden yazılır).
run_cpcv_study CPCV'yi yalnız runnable iken çalıştırır; hesap sırasında
oluşan ValueError (tanımsız Sharpe, < 2 train satırı) "cpcv.computation"
failed kontrolü olarak raporlanır, istisna olarak fırlatılmaz. Tip hataları
fırlatılır.
```

**17.2.28 İleri Yönlü Outcome Horizon ve Paylaşılan Pencere Purge'ü (LOCKED)**

```
Outcome horizon (bir kararın pozisyonunun getiri üretmeye devam ettiği süre)
hiçbir yerde kayıtlı DEĞİLDİR (BacktestResult'ta fill zamanı yok) ve
getirilerden TAHMİN EDİLMEZ. Kayıtlı olan: her matris satırının rolling
backtest penceresi (window_indices). Her pencere düz başlayan bağımsız bir
backtest'tir: bir outcome pencere sınırını geçemez. Dolayısıyla:
  - hiçbir fold sınırı bir pencerenin içine düşmüyorsa -> passed (kanal kapalı)
  - düşüyorsa ve purge kapalıysa -> failed ("horizon unknown")
  - düşüyorsa ve purge_shared_backtest_windows=True -> passed: her bölünmede
    bir test satırıyla AYNI backtest penceresini paylaşan her train satırı
    seçimden düşer (muhafazakâr; kayıtsız tutma süresi pencere sonunda biter).
compute_cpcv_paths(..., purge_shared_backtest_windows=False) additive;
varsayılan eski davranış; CpcvSplitResult.purged_train_row_count (varsayılan 0).
```

**17.2.29 Geri Yönlü Lookback (LOCKED)**

```
Lookback (bir pencerenin policy'sinin okuyabildiği geçmiş) yalnız çağıranın
ContextAwareWindow'larında (context_start) vardır; WindowResult onu taşımaz.
lookback_windows verilmezse -> not_evaluated (ASLA sıfır varsayılmaz;
context_start == evaluation.start sıfır bağlamı açıkça beyan eder).
Verilirse: evaluation alanları değerlendirme pencereleriyle birebir aynı
olmalı (aksi failed). Kesin kontrol, bölünme başına: train satırı katkı
veren her pencerenin [context_start, evaluation.start) aralığı hiçbir test
grubuyla örtüşmemeli; örtüşme -> failed (ilk ihlal: bölünme, pencere, test
grubu, bağlam süresi, embargo, en büyük lookback). Embargo mekanizmadır;
kontrol embargo'nun yeterli olup olmadığını ölçer.
```

**17.2.30 Kanıt:** §28.R (gerçek SQLite -> sıfır bağlamlı ve context-aware rolling -> TrialGroup -> çalışma).

**17.2.31 Tüketiciler:** `cpcv_study.py`, `cpcv.py` ve `combinatorial_folds.py`'nin tasarlanmış tek tüketicisidir (import yönü testleri bunu listeler).

**17.2.32 Gözlem Düzeyi (t1) Purging İçin Veri Gereksinimi** [2026-09-25 notu: gereksinim §17.2.34–17.2.37'de, BacktestResult DEĞİŞTİRİLMEDEN, eklemeli bir gözlemciyle karşılandı; aşağıdaki metin tarihseldir.]

```
Klasik gözlem düzeyi purging, her karar/pozisyon için (giriş zamanı, çıkış
zamanı) aralıklarını gerektirir. Bugün BacktestResult yalnız fill_count /
trade_count + equity eğrisi taşır. Gerekli en küçük additive veri: backtest
motorunun pencere başına pozisyon aralıklarını (entry_time, exit_time |
açık-sonda) kaydetmesi ve bunun WindowResult'a/Trial'a taşınması — Faz 4
sonuç sözleşmesinin additive genişlemesi (BACKTEST_SPEC kararı). Bu yokken
çapraz-fold outcome örtüşmesi yalnız pencere hizalaması veya paylaşılan
pencere purge'ü ile dışlanır; CPCV sonucu "tam doğrulama" DEĞİLDİR.
```

**17.2.33 Kanıtlamadıkları:** kârlılık, bağımsızlık, gözlem düzeyi purging, tam CPCV doğrulaması, canlı onay.

**17.2.34 Pozisyon Aralığı Provenance'ı (LOCKED VE IMPLEMENTED + TESTED; §28.X)**

```
backtest/position_log.py (YENİ): PositionInterval(side LONG|SHORT, quantity,
  entry_time, exit_time | None); PositionIntervalRecorder.on_fill(...).
run_backtest_replay / run_backtest_from_store: position_observer=None
  (keyword-only, eklemeli). Pozisyonu değiştiren her fill'den sonra
...
Çatışma kaydı: aralıkları BacktestResult'a yeni alan olarak eklemek Faz 4
  sonuç sözleşmesini ve tam eşitlik karşılaştıran golden regression'ları
  bozardı; bu yüzden YAPILMADI. Seçenekler (kullanıcı kararı gerekirse):
  (a) mevcut gözlemci yolu (uygulandı), (b) BacktestResult'a alan + golden
  beklentilerinin güncellenmesi, (c) WindowResult/Trial'a ayrı provenance
  alanı (Bölüm 21 sözleşmesini değiştirir).
```

**17.2.35 Gözlem Düzeyi Purge (LOCKED VE IMPLEMENTED + TESTED)**

```
compute_cpcv_paths(..., position_intervals=None): {candidate_id: (pencere
  bloğu 0 aralıkları, blok 1, ...)}. Bir train satırı t, herhangi bir adayın
  t'nin getirisini üreten aralığı bir test grubunda da getiri üretiyorsa
  düşer (AFML label örtüşmesi; iki yön; adaylar üzerinde birleşim — tüm
  adaylar aynı satırlarda karşılaştırılır). Aralık giriş/çıkış anları kendi
  pencere bloğunun equity işaretleri olmalı (aksi hata). Varsayılan None:
  eski davranış.
run_cpcv_study(..., position_intervals=None): verilirse outcome kontrolü
  pencere içi fold sınırlarında passed ("observation-level purging") ve yeni
  "outcome_horizon.position_intervals" kontrolü: aralıklar her denemenin her
  penceredeki kayıtlı trade_count'unu yeniden üretmeli (aksi failed).
Paylaşılan pencere purge'ü (17.2.28) kaba üst sınır olarak kalır.
```
...
**17.4.18 Efektif-N ve Çok Pencereli DSR — LOCKED VE IMPLEMENTED + TESTED (2026-09-25; §28.U)** [17.4.14'teki iki deferred maddeden ilk ikisi.]

**17.4.19 Efektif-N:** `validation/trial_statistics.py` `estimate_effective_trial_count(matrix)`: DSR makalesi Ek A.3 ortalama-korelasyon interpolasyonu `N_eff = ρ̄ + (1 − ρ̄)·M`, ρ̄ = hizalı TrialReturnMatrix kolonlarının ikili Pearson korelasyonlarının ortalaması (80 basamak). ρ̄ < 0 -> N_eff > M, `exceeds_trial_count` ile işaretlenir, KIRPILMAZ ("kötü koşulluluk" kararı: raporla, düzeltme uydurma). Sıfır varyanslı kolon -> hata. N_eff DSR'ye OTOMATİK verilmez (DSR'nin N'i çağıran beyanı kalır, §17.4.2). Entropi/kümeleme yöntemleri kapsam dışı.

**17.4.20 Çok pencereli DSR:** `compute_pooled_deflated_sharpe_ratio(group, *, selected_candidate_id, independent_trial_count, risk_free_per_period)`: kilitli `_deflated_sharpe_from_statistics` aynen; her denemenin pencereleri periyodik getirileri birleştirilerek havuzlanır (her pencere kendi başlangıç nakdinden başlar: sermaye sıfırlaması getirilerin içindedir); Sharpe = Stage-2-özdeş alt örneklem Sharpe'ı (PBO yardımcısı), momentler DSR'nin popülasyon momentleri; havuzlanmış uzunluklar eşit olmalı. Varsayım DSR ile aynı (i.i.d.), pencere sınırlarına genişletilmiş.

**17.4.21 Kanıt:** §28.U — elle korelasyonlar (N = 2, 1, 3 ve 3 − 2√2/3); tek pencerede kilitli DSR ile BİREBİR; iki pencerede eşdeğer tek pencere kanıtıyla 1e-20 içinde eşit.

**17.4.22 Açık (17.4.14'ten kalan):** Harvey–Liu alternatif eşiği (ayrı yöntem), güven eşiği/geçti-kaldı (risk politikası — kullanıcı kararı), yıllıklandırılmış DSR raporlaması.
...
**17.5.25 PBO Yan İstatistikleri — LOCKED VE IMPLEMENTED + TESTED (2026-09-25; §28.V)**

`validation/pbo_diagnostics.py` `compute_pbo_diagnostics(matrix, *, block_count, risk_free_per_period)`: aynı CSCV kombinasyonları ve IS seçimleri (önce `compute_probability_of_backtest_overfitting` çağrılır; doğrulama ve seçim özdeş).

**17.5.26 Performans bozulması (§3.2):** IS-seçilenin OOS Sharpe'ının IS Sharpe'ına ağırlıklı en küçük kareler doğrusu `OOS = intercept + slope·IS`; eşit IS kazananları kombinasyon ağırlığını paylaşır (§17.5.15.5); sabit IS Sharpe -> eğim/kesişim None (0 değil).

**17.5.27 Kayıp olasılığı (§3.3):** seçilen adayın OOS Sharpe'ı < 0 olan kombinasyonların ağırlık payı, tam kesir.

**17.5.28 Stokastik baskınlık (§3.4):** `not_evaluated` — tanım bu teslimatta birincil kaynaktan doğrulanmadı; uydurulmadı.

**17.5.29 Kanıt:** §28.V — elle eğim −5, kesişim 6√2, P(kayıp) 1/2; eşitlikle 1/4; sabit IS -> None; gerçek rolling grubunda PBO seçimleriyle tutarlı.

**17.5.30 Kanıtlamadıkları:** eşik/geçti-kaldı, seçim, kârlılık.

### 17.6 Multiple-Testing Corrections — Holm Düzeltme Temeli (§17.6.1–17.6.10, §28.N — 18/18) ve Tek Yönlü Sharpe p-Değerleri + Aile Kapsamı (§17.6.11–17.6.18, §28.S — 9/9) LOCKED VE IMPLEMENTED + TESTED; Sonuç Seçim Politikası AÇIK
...
**17.6.11 p-Değeri Üretimi ve Aile Kapsamı — LOCKED VE IMPLEMENTED + TESTED (2026-09-25; §28.S)**

[2026-09-25 notu: 17.6.10 madde 1 ve 2 aşağıda karara bağlandı; madde 3 (sonuç seçimi) bilinçli olarak AÇIK kalır — sonuçlar hiçbir karara girmez.]

**17.6.12 Test istatistiği (LOCKED)**

```
Modül: src/crypto_quant_lab/validation/sharpe_significance.py (YENİ)
PSR (Bailey & López de Prado 2012) SR* = 0'da — kilitli DSR z'si (§17.4)
SR0 = 0 ile:
  z = SR * sqrt(T - 1) / sqrt(1 - skew * SR + (kurt - 1) / 4 * SR^2)
  p = 1 - Phi(z);  H0: per-observation Sharpe (rf fazlası) <= 0, TEK YÖNLÜ
SR = değişmemiş Stage-2 Sharpe; T = periyodik getiri sayısı; skew/kurt = DSR'nin
popülasyon momentleri; Phi = DSR'nin Decimal normal CDF'i (aynı nesne,
yeniden kullanım). 80 basamaklı DSR context'i, 28 basamaklı çıktı.
```

**17.6.13 Varsayım ve sınırlar (LOCKED)**

```
Getiriler i.i.d. kabul edilir; seri korelasyon DÜZELTİLMEZ, lag-1
otokorelasyon yalnız raporlanır (bir okuyucu varsayımın şüpheli olduğunu
görebilsin). Yalnız tek pencereli denemeler (DSR ile aynı); çok pencereli
deneme hiçbir hesaptan önce reddedilir. Stage-2'nin tanımsız Sharpe hatası
aynen yayılır; standart hata terimi <= 0 ise ValueError.
```

**17.6.14 Aile kapsamı (LOCKED)**

```
evaluate_sharpe_family(group, *, significance_level, risk_free_per_period=0):
aile = TrialGroup'ta KAYITLI her deneme (m = n, n > m kapalı kalır);
family_id = group_id; mevcut apply_holm_correction uygulanır. Kaydedilmemiş
veya başarısız denemeler ailede yoktur: Holm düzeltmesi tam bir araştırma
geçmişinin gerektireceğinin ALT SINIRIDIR (FAMILY_SCOPE metni sonuçta).
```

**17.6.15 Sonuç:** `SharpeSignificance` (candidate_id, sample_length, sharpe_ratio, skewness, kurtosis, z_statistic, p_value ∈ [0,1], lag1_autocorrelation), `SharpeFamilyTest` (family_id, null_hypothesis, method, assumption, family_scope, results, holm).

**17.6.16 Kanıt:** §28.S.

**17.6.17 Kanıtlamadıkları:** otokorelasyona dayanıklı anlamlılık, çok pencereli anlamlılık, tam araştırma geçmişi, strateji onayı.

**17.6.18 Açık:** sonuç seçim/raporlama politikası (17.6.10 madde 3); otokorelasyon düzeltmesi (ör. HAC standart hatası) ve çok pencereli birleştirme ayrı kontrat.

**17.6.19 Seri Bağımlılığa Dayanıklı ve Çok Pencereli Sharpe p-Değerleri — LOCKED VE IMPLEMENTED + TESTED (2026-09-25; §28.Y)** [17.6.18'deki otokorelasyon ve çok pencere maddeleri.]

**17.6.20 Yöntem:** `compute_hac_sharpe_significance(group, *, lag=None, risk_free_per_period)`: Lo (2002, "The Statistics of Sharpe Ratios") GMM/delta yöntemi; momentler `(r − μ, (r − μ)² − σ²)`, Newey–West (1987) Bartlett HAC kovaryansı `Σ = Γ0 + Σ_{j≤q} (1 − j/(q+1))(Γj + Γjᵀ)`; `SR = (μ − rf)/σ` (popülasyon σ), `V = ∇g Σ ∇gᵀ`, `z = SR·√T/√V`, `p = 1 − Φ(z)` (DSR'nin Decimal Φ'si), H0: Sharpe ≤ 0, tek yönlü. Varsayılan lag `⌊4(T/100)^(2/9)⌋` (Newey–West 1994), Decimal-only. q = 0'da V, PSR varyans terimine `1 − γ3·SR + (κ−1)/4·SR²` indirgenir.

**17.6.21 Çok pencere:** getiriler pencere sırasıyla birleştirilir; otokovaryans çiftleri pencere sınırını ASLA geçmez.

**17.6.22 Geçerlilik ve "değerlendirilemedi":** varsayımlar (durağan, ergodik, sonlu dördüncü moment, asimptotik) sonuçta yazılıdır. `T ≤ 2(q+1)` veya pozitif olmayan V / sabit getiri -> `status = "not_evaluated"`, neden yazılır, p YOK. `evaluate_hac_sharpe_family`: herhangi bir deneme değerlendirilemezse aile düzeltilmez (`holm = None`, kısmi aile düzeltmeyi küçültürdü). i.i.d. modülü (17.6.11) değişmedi; genel piyasa doğrulaması olarak sunulmaz.
...
[2026-09-25 notu: yukarıdaki paragraf tarihseldir. "Komşu parametre konfigürasyonları" önkoşulu, üretilmeden, yalnız bir TrialGroup'ta gerçekten değerlendirilmiş adaylardan karşılandı (§17.7.1–17.7.8). Projede gerçek parametrik adaylar vardır: research/funding_carry.py `funding_carry_candidate`.]

**17.7.1 Kapsam (LOCKED):** `src/crypto_quant_lab/validation/parameter_stability.py` (YENİ), `map_parameter_neighborhoods(group, *, risk_free_per_period=Decimal(0)) -> ParameterNeighborhoodMap`. Betimsel: skor, tolerans, plato kriteri, sıralama, seçim, optimizer YOK.

**17.7.2 Izgara (LOCKED):** Tüm adaylar aynı parametre anahtar kümesini beyan etmeli; aynı parametreli iki aday reddedilir (belirsiz ızgara noktası). Tüm değerleri int (bool değil) veya sonlu Decimal olan anahtar SIRALI eksendir; diğerleri (str, bool, None, tuple) kategoriktir ve komşular arasında birebir eşit olmalıdır. En az 2 aday ve en az 1 sıralı eksen gerekir.

**17.7.3 Komşu (LOCKED):** c'nin k eksenindeki alt/üst komşusu: diğer tüm anahtarları c ile eşit olan ve k değeri grupta bulunan en yakın küçük/büyük değer olan aday. Komşu yoksa kayıt yoktur (uç nokta).

**17.7.4 Metrik girdisi (LOCKED):** her adayın değişmemiş Stage-2 Sharpe'ı; yalnız tek pencereli denemeler (DSR/p-değeri ile aynı); Stage-2 hataları aynen yayılır.

**17.7.5 Sonuç:** `ParameterNeighborhoodMap(scope, ordered_parameters, categorical_parameters, candidates)`; `CandidateNeighborhood(candidate_id, parameters, sharpe_ratio, neighbors)`; `ParameterNeighbor(parameter, direction ∈ {lower, upper}, candidate_id, value, sharpe_ratio)`.

**17.7.6 Kanıt:** §28.T.

**17.7.7 Kanıtlamadıkları:** parametre stabilitesi, plato, aşırı uyumun yokluğu; seçim.

**17.7.8 KARAR VERİLDİ (2026-09-25, kullanıcı: (a) + (c), yalnız betimsel ve eşiksiz; §17.7.9–17.7.12) — tarihsel metin: Stabilite metriği:** komşuluk üzerinden hangi ölçünün "stabil" sayılacağı (ör. komşu Sharpe'larının en küçüğü/ortalaması ile kendi Sharpe'ı arasındaki fark, göreli düşüş, işaret tutarlılığı, bir tolerans) bir yöntem/kullanıcı kararıdır; eşik de risk politikasıdır. Karar verilene kadar harita yalnız betimseldir.


**17.7.9 Stabilite Ölçüleri (LOCKED VE IMPLEMENTED + TESTED; §28.W):** `describe_neighborhood_stability(neighborhood_map)` her aday için (a) `sharpe_minus_min_neighbor` = kendi Sharpe − değerlendirilmiş komşuların en küçük Sharpe'ı (80 basamak, tam), (c) işaret tutarlılığı (işaret −1/0/+1; sıfır yalnız tam sıfır komşuyla eşleşir; `same_sign_neighbor_count`, `different_sign_neighbor_count`, `sign_consistent`).

**17.7.10 Tanımlar:** komşusuz aday -> iki ölçü de None, not "undefined" (0 veya True DEĞİL); negatif Sharpe aritmetik olarak aynı; eşit değerler -> fark tam 0.

**17.7.11 Kapsam:** betimsel, EŞİKSİZ; stabilite hükmü, risk skoru, seçim, geçti/kaldı YOK (STABILITY_SCOPE metni).

**17.7.12 Kanıt:** §28.W — elle negatif/sıfır/eşit/komşusuz vakalar, uzun Decimal farkının tamlığı, gerçek parametrik ailede uçtan uca.
...
FAZ6C — CPCV PATH DISTRIBUTION SUMMARY — TAMAMLANDI (2026-09-25):
  §17.2.20–17.2.24 kilitlendi; cpcv.py'ye additive summarize_cpcv_paths /
  CpcvPathSummary (betimsel; eşik/p-değeri/geçti-kaldı yok; path'lerin
  gözlem paylaşımı ve purging sınırı sonuçta görünür). Test:
  tests/test_validation_cpcv.py (+8, toplam 34). §28.Q 10/10. Ufuk kaynak
  incelemesi ve sonraki dilim önerisi §17.2.25 (kod yok). CPCV bütünü,
  FAZ6C ve Faz 6 NOT COMPLETE.

FAZ6C — OFFLINE CPCV STUDY + INPUT ASSESSMENT, SHARPE P-VALUES + FAMILY,
PARAMETER NEIGHBORHOOD MAP — TAMAMLANDI (2026-09-25, tek oturum, ayrı
commit'ler 2ce960f, 3bcfdd8, ac13f83, a4d657e):
  cpcv_study.py: TrialGroup -> matrix -> pencere-hizalı fold -> uygunluk
  denetimi (provenance, satır sahipliği, ileri outcome horizon, geri
  lookback; passed/failed/not_evaluated + neden) -> path -> özet; cpcv.py'ye
  paylaşılan pencere purge'ü (varsayılan kapalı). sharpe_significance.py:
  PSR(SR* = 0) tek yönlü p-değeri (DSR Phi/momentleri) + TrialGroup ailesi
  üzerinde mevcut Holm. parameter_stability.py: değerlendirilmiş ızgaradan
  betimsel komşuluk haritası. Testler: test_validation_cpcv_study.py (13),
  test_validation_sharpe_significance.py (9),
  test_validation_parameter_stability.py (4). §28.R 12/12, §28.S 9/9,
  §28.T 6/6. FAZ6C ve Faz 6 NOT COMPLETE.

FAZ6C — KALAN BOŞLUKLAR PAKETİ — TAMAMLANDI (2026-09-25; commit'ler
9379daf, 6d1830d, 432a3b6, eef5ed8, f56768d, ff82b79):
  stabilite ölçüleri (a)+(c) betimsel; pozisyon aralığı provenance'ı
  (eklemeli gözlemci, BacktestResult değişmedi) + CPCV gözlem düzeyi purge'ü;
  Lo (2002)/Newey–West HAC ve çok pencereli Sharpe p-değerleri (kesin
...
+ §17.6.10):
  ...
  3 PBO stokastik baskınlık (§17.5.28) — birincil kaynak doğrulaması
  4 Harvey–Liu eşiği, yıllıklandırılmış DSR raporu (§17.4.22) — ayrı yöntem
  5 aralıkların sonuç sözleşmesine taşınması (§17.2.34) — isteğe bağlı karar
  Kapsam dışı (başlatılmaz): aday seçimi, optimizer, final holdout.

Sonraki (henüz başlanmadı):
  FAZ6C'nin kalanları: yukarıdaki 1–5.
  Deferred (tarihsel liste; 2026-09-25'te efektif-N, çok pencereli DSR
  ve PBO §3.2–3.3 kapandı): PBO §3.4.
...
### 28.Q — CPCV PATH DISTRIBUTION SUMMARY ACCEPTANCE (10/10 IMPLEMENTATION/TEST EXERCISED)

Bölüm 17.2.20–17.2.24'ün `summarize_cpcv_paths` tarafından karşılandığını kaydeder; `tests/test_validation_cpcv.py`'ye 8 yeni test (dosyada toplam 34, tümü PASS).

1. Public semboller `CpcvPathSummary`, `summarize_cpcv_paths`, `DIAGNOSTIC_SCOPE` eklendi; mevcut `compute_cpcv_paths` API'si aynı (§17.2.20). **PASS** — `test_public_api_and_reuse_of_the_pbo_rules`.
2. Elle türetilmiş fixture: path Sharpe'ları (5/2)√(3/17), (11/4)√(12/59), (3/4)√6; min, max, medyan (orta) ve bağımsız referansla ortalama/stdev (§17.2.21). **PASS** — `test_summary_of_the_hand_derived_fixture`.
3. Sonuç, offline teşhis / bağımsız olmayan path / purging yok / eşik yok / tam CPCV değil / canlı onay değil ifadelerini ve bayraklarını taşır (§17.2.20). **PASS** — aynı test.
4. Sharpe'a göre artan sıralama ve eşitlik sayımı (1, 0, 2; 3 path) (§17.2.21). **PASS** — `test_summary_orders_paths_by_sharpe_and_counts_ties`.
5. Eşit Sharpe'larda sıra korunur; stdev tam 0; ortak satırlar sayılır (4) (§17.2.21). **PASS** — `test_equal_path_sharpes_keep_path_order_and_zero_dispersion`.
6. Çift P'de medyan iki ortadakinin ortalamasıdır (P = 4) (§17.2.21). **PASS** — `test_even_path_count_median_is_the_mean_of_the_two_middle_values`.
7. Tek path dağılım değildir: exact hata (§17.2.22 adım 2). **PASS** — `test_a_single_path_is_not_a_distribution`.
8. Tip, path sırası ve path uzunluğu exact mesajlarla (§17.2.22). **PASS** — `test_invalid_summary_inputs_have_exact_messages`.
9. Ambient context etkisiz; deterministik; girdi değişmez (§17.2.21). **PASS** — `test_summary_is_context_independent_deterministic_and_pure`.
10. Model purging iddiasını ve sonlu olmayan değerleri reddeder (§17.2.21). **PASS** — `test_summary_model_refuses_a_purging_claim_and_non_finite_values`.

**CPCV path distribution summary acceptance count: 10 / 10 implementation/test exercised.** Bu, label/outcome-horizon purging'in, istatistiksel çıkarımın, eşiğin, "CPCV bütünü"nün, FAZ6C'nin veya Faz 6'nın tamamlandığı anlamına GELMEZ.

### 28.R — OFFLINE CPCV STUDY + INPUT ASSESSMENT ACCEPTANCE (12/12 IMPLEMENTATION/TEST EXERCISED)

`tests/test_validation_cpcv_study.py` (13 test, tümü PASS). Gerçek SQLite + `run_rolling_backtest_from_store` / `run_context_aware_rolling_backtest_from_store` -> TrialGroup.

1. Pencere-hizalı gruplar boşlukları önceki gruba katar; tip/sayı/örtüşme retleri (§17.2.26). **PASS** — `test_window_aligned_groups_absorb_gaps_into_the_preceding_group`.
2. Sıfır bağlam beyanıyla dört kontrol passed; çalışma, doğrudan zincirle (matrix, fold model, compute_cpcv_paths, summarize) birebir aynı; 3 path x 16 gerçek satır; her satır seçimle tutarlı (§17.2.26–17.2.27). **PASS** — `test_real_chain_zero_context_is_fully_evaluated_and_matches_the_direct_path`.
3. Lookback verilmezse not_evaluated ve nedeni; çalışır ama tam değerlendirilmiş değildir (§17.2.29). **PASS** — `test_missing_lookback_is_reported_not_assumed`.
4. Gerçek 1 saatlik bağlam + embargo 0: failed, ilk ihlal elle türetilmiş mesajla, sonuç yok (§17.2.29). **PASS** — `test_real_lookback_without_embargo_fails_and_blocks_the_study`.
5. Embargo = lookback: passed; embargo yapısı elle; 4 path (§17.2.29). **PASS** — `test_embargo_covering_the_lookback_passes_and_runs`.
6. Değerlendirilen pencereleri tarif etmeyen lookback pencereleri failed (§17.2.29). **PASS** — `test_lookback_windows_must_describe_the_evaluated_windows`.
7. Pencere içine düşen fold sınırı purge kapalıyken failed (§17.2.28). **PASS** — `test_fold_boundary_inside_a_backtest_window_fails_without_the_purge`.
8. Paylaşılan pencere purge'ü: elle sayımlar 0/12, 4/8, 4/8; 7 path (§17.2.28). **PASS** — `test_shared_window_purge_drops_training_rows_sharing_a_window_with_test_rows`.
9. Purge'ün train'i tükettiği bölünme "cpcv.computation" failed olarak raporlanır, fırlatılmaz (§17.2.27). **PASS** — `test_purge_leaving_too_few_training_rows_is_reported_not_raised`.
10. Provenance uyuşmazlığı sonraki kontrolleri not_evaluated yapar; satır düzeyi mesaj (§17.2.27). **PASS** — `test_provenance_mismatch_stops_the_later_checks`.
11. Fold grubu dışındaki satır ownership failed (§17.2.27). **PASS** — `test_rows_outside_the_fold_groups_fail_the_ownership_check`.
12. Argüman/durum doğrulaması; modül offline ve package-root'tan export edilmez; cpcv/combinatorial_folds import yönü testleri tüketicileri tam listeler (§17.2.31). **PASS** — `test_invalid_arguments_and_check_statuses`, `test_module_is_offline_and_not_exported`, iki import yönü testi.

**Count: 12 / 12.** Gözlem düzeyi purging, tam CPCV doğrulaması veya FAZ6C'nin tamamlanması anlamına GELMEZ.

### 28.S — SHARPE P-VALUES + FAMILY ACCEPTANCE (9/9 IMPLEMENTATION/TEST EXERCISED)

`tests/test_validation_sharpe_significance.py` (9 test, tümü PASS).

1. Elle: getiriler 0.03/−0.01 -> SR = √3/4, skew 0, kurt 1, z = 3/4, p = 1 − Φ(0.75) (math.erf ile, < 1e-15), lag-1 otokorelasyon −3/4 (§17.6.12–17.6.13). **PASS** — `test_hand_case_z_three_quarters`.
2. Sıfır ortalama -> SR = 0, z = 0, p = 1/2 (§17.6.12). **PASS** — `test_zero_mean_gives_p_one_half`.
3. Holm ailesi elle: düzeltilmiş 2p ve 0.5, ret yok; varsayım/aile kapsamı/tek yönlü metinleri (§17.6.14). **PASS** — `test_holm_family_over_the_recorded_trials`.
4. rf null'ı kaydırır (fazla ortalama 0 -> p = 1/2) (§17.6.12). **PASS** — `test_risk_free_rate_moves_the_null`.
5. Çok pencereli deneme hiçbir hesaptan önce exact mesajla reddedilir (§17.6.13). **PASS** — `test_multi_window_trials_are_rejected_before_any_computation`.
6. Tip/rf/tanımsız Sharpe/geçersiz α retleri (§17.6.13). **PASS** — `test_invalid_arguments_and_undefined_statistics`.
7. p ∈ [0, 1] model doğrulaması; ambient context etkisiz (§17.6.15). **PASS** — `test_model_and_context_independence`.
8. DSR'nin Φ'si aynı nesne; seçim/sıralama adı yok; float/math yok (§17.6.12). **PASS** — `test_reuses_the_locked_dsr_normal_cdf_and_selects_nothing`.
9. Gerçek rolling grubu: z ve p raporlanan istatistiklerden bağımsız olarak yeniden hesaplanır; Holm ≥ ham p; deterministik (§17.6.14). **PASS** — `test_real_rolling_group_family`.

**Count: 9 / 9.** Otokorelasyona dayanıklı veya çok pencereli anlamlılık, sonuç seçim politikası veya strateji onayı anlamına GELMEZ.

### 28.T — PARAMETER NEIGHBORHOOD MAP ACCEPTANCE (6/6 IMPLEMENTATION/TEST EXERCISED)

`tests/test_validation_parameter_stability.py` (4 test, tümü PASS). Gerçek parametrik momentum ailesi SQLite + rolling üzerinden.

1. Sıralı (threshold) ve kategorik (side) ayrımı; komşuluk yapısı elle; taraflar birbirine komşu olmaz (§17.7.2–17.7.3). **PASS** — `test_real_parametric_family_neighbors`.
2. Her Sharpe adayın kendi Stage-2 değeri, komşu kayıtlarında da (§17.7.4). **PASS** — aynı test.
3. İki sıralı eksen; köşe noktası iki üst komşu (§17.7.3). **PASS** — `test_two_ordered_axes`.
4. < 2 aday, farklı anahtar kümesi, tekrarlanan ızgara noktası, sıralı eksen yokluğu, çok pencere, tip, yön retleri (§17.7.2, 17.7.4). **PASS** — `test_invalid_groups_are_refused`.
5. Deterministik; modülde seçim/skor/optimize adı yok (§17.7.1). **PASS** — `test_deterministic_and_selects_nothing`.
6. Kapsam metni "no stability score" içerir (§17.7.1). **PASS** — `test_real_parametric_family_neighbors`.

**Count: 6 / 6.** Parametre stabilitesi, plato veya seçim anlamına GELMEZ; stabilite metriği AÇIK (§17.7.8).

### 28.U — EFFECTIVE N + POOLED DSR ACCEPTANCE (5/5)

`tests/test_validation_trial_statistics.py` (5 test, PASS). 1. elle korelasyonlar N = 2 / 1 / 3 (işaretli) / 3 − 2√2/3 — `test_effective_n_hand_cases`. 2. sıfır varyans, < 2 deneme, tip retleri — `test_effective_n_invalid_inputs`. 3. tek pencerede kilitli DSR ile birebir (N = 2, 5, 50) — `test_pooled_dsr_equals_the_locked_dsr_for_single_window_trials`. 4. iki pencere = eşdeğer tek pencere DSR (1e-20) — `test_pooled_dsr_matches_dsr_on_the_equivalent_single_window_evidence`. 5. argüman retleri — `test_pooled_dsr_invalid_inputs`. **5/5.**

### 28.V — PBO SIDE STATISTICS ACCEPTANCE (5/5)

`tests/test_validation_pbo_diagnostics.py` (5 test, PASS). 1. elle eğim −5, kesişim 6√2, P(kayıp) 1/2, stokastik baskınlık not_evaluated — `test_hand_case_degradation_line_and_probability_of_loss`. 2. eşitlik ağırlığı -> 1/4 — `test_tied_in_sample_winners_share_the_combination_weight`. 3. sabit IS -> None — `test_constant_in_sample_sharpe_leaves_the_slope_undefined`. 4. PBO doğrulaması aynen — `test_validation_is_the_pbo_validation`. 5. gerçek rolling grubu PBO seçimleriyle tutarlı, deterministik — `test_real_rolling_group_matches_the_pbo_selections`. **5/5.**

### 28.W — STABILITY MEASURES ACCEPTANCE (3/3)

`tests/test_validation_parameter_stability.py` (+3 test; dosyada 7, PASS). 1. negatif/sıfır/eşit/komşusuz elle — `test_hand_cases_gap_and_sign_consistency`. 2. uzun Decimal farkı tam — `test_gap_is_exact_for_long_decimals`. 3. gerçek parametrik ailede uçtan uca + tip retti — `test_measures_on_the_real_parametric_family`. **3/3.**

### 28.X — POSITION INTERVALS + OBSERVATION-LEVEL PURGE ACCEPTANCE (7/7)

`tests/test_backtest_position_log.py` (4), `tests/test_validation_cpcv.py` (+2), `tests/test_validation_cpcv_study.py` (+2), PASS. 1. gözlemcili = gözlemcisiz sonuç; elle aralıklar; geçişler = trade_count — `test_observer_never_changes_the_result_and_records_hand_derived_intervals`. 2. işaret zamanlaması — `test_produces_return_at_follows_the_mark_timing`. 3. kayıt/aralık doğrulaması — `test_recorder_and_interval_validation`. 4. rolling "with positions" değişmemiş WindowResult'lar, sıfır ve context-aware — `test_rolling_with_positions_returns_the_unchanged_window_results`. 5. elle purge 2,2,0,0,2,2; pencere purge'ü aynı vakayı boşaltır — `test_recorded_position_intervals_purge_exactly_the_overlapping_training_rows`. 6. provenance retleri — `test_position_interval_provenance_is_checked`. 7. gerçek zincir: bağımsız türetmeyle eşit, pencere purge'ünden ince; oynanmış aralık failed — `test_recorded_intervals_purge_the_real_chain_more_finely_than_the_window_purge`, `test_intervals_that_do_not_reproduce_the_trade_count_fail`. Tüm backtest/golden suite'leri değişmeden geçer. **7/7.**
