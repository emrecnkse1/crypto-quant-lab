...
- **Faz 6:** Validation / anti-overfitting — **Kısmen tamamlandı** (ayrıntı: `VALIDATION_SPEC.md` §22.2)
  ...
  - FAZ6C (advanced overfitting controls): Tamamlanmadı — purging/embargo, trial-group, Deflated Sharpe, trial return matrix, PBO/CSCV, Holm düzeltme temeli ve CPCV fold modeli, train-seçimli path getirileri, betimsel path dağılımı özeti ve girdi uygunluk denetimli offline CPCV çalışması (paylaşılan pencere purge'ü dahil), tek yönlü Sharpe p-değerleri + TrialGroup ailesi üzerinde Holm, betimsel parametre komşuluk haritası (2026-09-25; yalnız offline teşhis) tamamlandı; gözlem düzeyi purging (pozisyon aralığı veri gereksinimi), sonuç seçim politikası, otokorelasyon düzeltmesi, stabilite metriği (kullanıcı kararı), geçerli p-değeri üretimi / aile kapsamı / seçim politikası, parameter stability, efektif-N, çok pencereli DSR ve PBO yan istatistikleri açık
