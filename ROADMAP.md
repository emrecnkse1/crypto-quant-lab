...
- **Faz 7:** İlk Funding/Basis araştırması — **Başladı; ilk üç dikey dilim tamamlandı, Faz 7 bütünü tamamlanmadı** (ayrıntı: `FUNDING_RESEARCH_SPEC.md`)
  ...
  - Çok bacaklı muhasebe/execution: kodla eşleştirilmiş TASLAK (DRAFT, FUNDING_RESEARCH_SPEC §19); ilk muhasebe dilimi IMPLEMENTED + TESTED (§19.10.1); E1 opt-in olay sıralaması + ilk in-memory replay (iki mum serisi, scripted intent'ler, MS9 funding sırası) + sentetik offline demo IMPLEMENTED + TESTED (§19.12.8, 2026-09-25); store-backed çok bacaklı runner NOT IMPLEMENTED; geri kalan tasarım ve K1–K7 kararları açık (R1–R5 yalnız ilk replay için kararlaştırıldı)
